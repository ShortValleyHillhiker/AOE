(() => {
  ...
  function initContextToggle() {
    ...
    const contextTrigger = document.getElementById("toggle-context");
    if (contextBlock && contextTrigger) {
      contextTrigger.addEventListener("click", () => {
        const isExpanded = contextBlock.style.maxHeight;
        if (isExpanded) {
          contextBlock.style.maxHeight = contextBlock.scrollHeight + "px";
          requestAnimationFrame(() => {
            contextBlock.style.maxHeight = "0px";
            setTimeout(() => {
              contextBlock.style.maxHeight = "";
            }, 300);
          });
          contextTrigger.classList.remove("active");
        } else {
          contextBlock.style.maxHeight = contextBlock.scrollHeight + "px";
          contextTrigger.classList.add("active");
        }
      });
    }
  }
